...
export function ErrorDetector({ issues, messages, toolCalls, metrics }: ErrorDetectorProps) {
  ...
  const handleAnalyze = async () => {
    const { credentials, region, modelId } = settings.bedrock;
    if (!credentials) {
      setAnalyzeError('Add Bedrock credentials in Settings first.');
      return;
    }
    setAnalyzing(true);
    setAnalyzeError(null);
    try {
      const analysis = await analyzeSession(credentials, region, modelId, {
        messages: messages || [],
        toolCalls: toolCalls || [],
        issues,
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="border border-dark-700 rounded-lg p-3 bg-dark-800/40">
        ...
        {analyzeError && <p className="mt-2 text-[11px] text-red-400">{analyzeError}</p>}
        ...
        {aiAnalysis && (
          <div className="mt-3 bg-dark-900 border border-dark-700 rounded-md p-3 max-h-80 overflow-y-auto">
            ...
            <div className="text-[11px] text-dark-200 leading-relaxed whitespace-pre-wrap">{aiAnalysis}</div>
          </div>
        )}
      </div>
    ...
    <div className={`border rounded-lg ${config.border} ${config.bg}`}>
      <button onClick={() => setExpanded(!expanded)} className="w-full text-left p-3">
        <div className="flex items-start gap-2">
          <span className={`text-sm flex-shrink-0 ${config.iconColor}`}>{config.icon}</span>
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-1 flex items-center gap-2 text-[10px] text-dark-500">
              ...
              {issue.orchestrationIteration && <span>· iter {issue.orchestrationIteration}</span>}
            </div>
          </div>
          {issue.context && (
            <svg
